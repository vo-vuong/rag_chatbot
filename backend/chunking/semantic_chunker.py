...
class SemanticChunker:
    ...
    def chunk_elements(
        ...
        **kwargs
        ...
        try:
            ...
            stats = self._generate_stats(elements, chunked_elements, start_time, image_paths)
    ...
    def _generate_stats(
        ...
        stats = {
            ...
            "compression_ratio": len(chunks) / len(original_elements) if original_elements else 0,
            "image_count": len(image_paths),
            "processing_time": time.time() - start_time
        }

        # Character statistics
        original_chars = sum(len(e.text) for e in original_elements if hasattr(e, 'text'))
        chunked_chars = sum(len(c.text) for c in chunks if hasattr(c, 'text'))

        stats.update({
            "original_characters": original_chars,
            "chunked_characters": chunked_chars,
            "character_compression_ratio": chunked_chars / original_chars if original_chars else 0
        })
