"""
Context Recall metric using RAGAS LLMContextRecall.

Measures how many relevant claims from the reference are captured in retrieved contexts.
Uses ground truth reference answer to determine what should have been retrieved.
"""
...
class ContextRecallMetric(GenerationMetric):
    """
    Context Recall metric using RAGAS LLMContextRecall.

    Evaluates how many of the relevant claims from the reference answer
    are successfully captured in the retrieved contexts.

    Formula:
        Context Recall = (Claims attributable to retrieved context) / (Total claims in reference)

    Higher scores indicate fewer relevant claims were missed.
    ...
    def __init__(self, model_name: str = "gpt-4o-mini"):
        ...
        Args:
            model_name: OpenAI model name for LLM-based claim verification
        """
        self.model_name = model_name
        self._scorer = None

    def _get_scorer(self):
        """Lazy initialization of RAGAS LLMContextRecall scorer."""
        if self._scorer is None:
            try:
                ...
                self._scorer = LLMContextRecall(llm=llm)
                logger.info(
                    f"Initialized ContextRecall scorer with LLM={self.model_name}"
                )
            except ImportError as e:
                raise ImportError(
                    "RAGAS and langchain-openai are required for ContextRecall. "
    ...
    async def calculate_query_score(
        ...
        """
        Calculate Context Recall score for a single query.
        ...
        Args:
            ...
            reference: Ground truth answer for claim extraction
        ...
        try:
            ...
            logger.error(f"ContextRecall calculation failed: {e}")
    ...
    def aggregate_scores(
        ...
        """
        Aggregate Context Recall scores across all queries.
